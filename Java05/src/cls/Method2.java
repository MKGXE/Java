...
public class Method2 {
	public static void main(String[] args) {
		...
		System.out.println("걀과는 : "+mc5.메서드4(5.5));		// 변수처럼 사용
...
class MyClass5{
	...
	void 메서드1() {
		// 입력값이 없으면 ()를 비워놓습니다.
		// 반환값이 업스면 자료형은 void로 하고 return은 생략
		// return;
	...
	// 메서드3 : 반환값O, 입력값X (코드 재사용 목적, 메서드 사용 후에 전달해줄 값이 있음)
	String 메서드3() {
	...
	int 메서드4(double dnum) {
		...
		double result = dnum - (int)dnum;		// 소수점 부분만 남음
		if (result >= 0.5) {
			dnum += 1.0;		// 만약에 소수점 부분이 0.5보다 컸다면 +1
		}
		
		return (int)dnum;		// 소수점 부분 날린다
	}
